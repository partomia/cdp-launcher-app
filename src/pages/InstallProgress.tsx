...
import { Loader2, XCircle, CheckCircle, StopCircle } from "lucide-react";
...
export function InstallProgress({ cluster, onClusterChange }: Props) {
  ...
  useEffect(() => {
    if (!isActive) return;
    const id = setInterval(() => setElapsed(elapsedSince(cluster.created_at)), 1000);
  ...
  async function handleCancel() {
    setCancelling(true);
    try { await installCancel(cluster.id); } catch {}
    setCancelling(false);
  }
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-6 py-4 border-b border-border/50 flex-shrink-0">
        ...
        {cluster.state === "failed" && <XCircle className="h-4 w-4 text-destructive flex-shrink-0" />}
        {cluster.state === "ready" && <CheckCircle className="h-4 w-4 text-green-500 flex-shrink-0" />}

        <h2 className="text-[16px] font-semibold">{cluster.name}</h2>
        <StateBadge state={cluster.state} />
        <span className="text-[12px] text-muted-foreground tabular-nums">{elapsed}</span>
